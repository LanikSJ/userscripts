...
(function () {
  ...
  // ---- Dynamically inserted promo blocks (merged from instacart-ad-remover.js) ----

  function removeAdBlocks() {
    // Target elements inside unified data placement containers
    const targetBlocks = document.querySelectorAll('div[data-placements="unified"] > div');

    targetBlocks.forEach(block => {
      // Check if the block contains a brand promo link or section
      if (block.querySelector('a[href*="/brands/"]') || block.querySelector('section')) {
        block.remove();
      }
    });
  }
  ...
  // Observe page updates to catch dynamically loaded ads (debounced via rAF)
  let rafScheduled = false;

  function scheduleRemoveAdBlocks() {
  ...
  function startAdBlockObserver() {
    new MutationObserver(scheduleRemoveAdBlocks).observe(document.body, {
      childList: true,
      subtree: true
    });
  }
  ...
  if (document.body) {
    ...
    document.addEventListener('DOMContentLoaded', startAdBlockObserver, { once: true });
  }
})();
